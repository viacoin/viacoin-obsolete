...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Viacoin address (e.g. %1)").arg("VsaBp3uF6kqsEmquiq1QWGM6x2kMQ1jCST");
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("viacoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "viacoin:");
    }
...
{
    QString ret = QString("viacoin:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Viacoin (%s).lnk", chain);
}
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "viacoinn.desktop";
    return GetAutostartDir() / strprintf("viacoin-%s.lnk", chain);
}
