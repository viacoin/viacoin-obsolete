...
Clone the git repositories for viacoin and gitian and then checkout the viacoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the viacoin repository
...
To build Viacoin (for Linux, OSX and Windows) just follow the steps under 'perform
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/viacoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/viacoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/viacoin-build.assert
```
...
[viacoin/gitian.sigs](https://github.com/viacoin/gitian.sigs/) repository.
